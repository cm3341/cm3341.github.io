...
After seeing some extreme temperatures in my initial plots, I read the dataset instructions more carefully, I discovered the `tmax` and `tmin` values appear ihn tenths of degrees C. To convert to true, interpretable values, I have to divide the values in these two variable columns by 10. I also converted the `prcp ` observations to millimeters (mm), as the available observations were originally in tenths of mm, according to the NY NOAA dataset instructions. 
